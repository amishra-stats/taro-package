# Setting 2: most abundant taxa/nodes are favoured
library(taro)
set.seed(123)
nsim <- 50
snr <- .25; xrho <- 0.5; nrank <- 3;
q <- 50; n = 300; intercept = 0.5
rho = 0.5; setting = 2; sparsity = 0.05
simulation_2 <- vector('list', nsim)
ii = 1; se_id <- 1
while (ii <= nsim) {
  tryCatch({
    set.seed(se_id*1234)
...
save(simulation_2, file = 'data/simulation2.RData')
...
control <- taro_control(alpha0 = 0.05, gamma0 = 0, spU = 0.5,
...
save(list = ls(), file = sprintf('output/simulation2_%s.RData',index))
...
# sink("simulation2.sh")
# for(i in 1:50){
#   cat(sprintf('bsub -W 3:00 -q short -n 1 -M 16 -R \'rusage[mem=6]\' -o out/out_%s -e err/err_%s \"Rscript simulation2.R %s 1>Rout/out_%s 2>Rerr/err_%s\"\n',i,i,i,i,i))
# }
# sink()
...
library(kableExtra)
kbl(out_table,format = 'latex',booktabs = T,  caption = "Setting 2", linesep = "") %>%
  pack_rows("Full data", 1,4) %>%
  pack_rows("Missing data", 5,8) %>%
  kable_styling(latex_options = "striped", stripe_index = c(5:8))


kbl(out_table,booktabs = T,  caption = "Setting 2", linesep = "") %>%
  pack_rows("Full data", 1,4) %>%
  pack_rows("Missing data", 5,8) %>%
  kable_styling(latex_options = "striped", stripe_index = c(5:8)) %>%
...
setEPS()
postscript('plots/setting2.eps', width = 10, height = 5)
plot_view
dev.off()
