# Setting 4:Higher level taxa are favoured to have a significant effect
rm(list = ls())
library(taro)
set.seed(123)
nsim <- 50
...
rho = 0.5; setting = 4; sparsity = 0.10
simulation_4 <- vector('list', nsim)
ii = 1; se_id <- 1
while (ii <= nsim) {
  tryCatch({
    set.seed(se_id*1234)
...
save(simulation_4, file = 'data/simulation4.RData')
...
control <- taro_control(alpha0 = -0.1, gamma0 = 1, spU = 0.5,
...
control <- taro_control(alpha0 = -0.1, gamma0 = 0, spU = 0.5,
                        inTol = 1e-5, inMaxIter = 300,
                        outMaxIter = 1000,outTol = 1e-8,
                        spV=0.5, lamMaxFac = 1e2, se1 = 1)
...
save(list = ls(), file = sprintf('output/simulation4_%s.RData',index))

# Code generating script to run simulation on server
# sink("simulation4.sh")
# for(i in 1:50){
#   cat(sprintf('bsub -W 3:00 -q short -n 1 -M 16 -R \'rusage[mem=6]\' -o out/out4_%s -e err/err4_%s \"Rscript simulation4.R %s 1>Rout/out4_%s 2>Rerr/err4_%s\"\n',i,i,i,i,i))
# }
# sink()
...
library(kableExtra)
kbl(out_table,format = 'latex',booktabs = T,  caption = "Setting 4", linesep = "") %>%
...
kbl(out_table,booktabs = T,  caption = "Setting 4", linesep = "") %>%
  pack_rows("Full data", 1,4) %>%
  pack_rows("Missing data", 5,8) %>%
  kable_styling(latex_options = "striped", stripe_index = c(5:8)) %>%
...
setEPS()
postscript('plots/setting4.eps', width = 10, height = 5)
plot_view
dev.off()
