# Setting 1: rare taxa is favoured
library(taro)
set.seed(123)
nsim <- 50
snr <- .25; xrho <- 0.5; nrank <- 3;
q <- 50; n = 300; intercept = 0.5
rho = 0.5; setting = 1; sparsity = 0.05
simulation_1 <- vector('list', nsim)
ii = 1; se_id <- 1
while (ii <= nsim) {
  tryCatch({
    set.seed(se_id*1234)
...
save(simulation_1, file = 'data/simulation1.RData')
...
rm(list = ls())
setwd('simulation_taro')
load('data/simulation1.RData')
...
nlambda = 100;PATH = T
control <- taro_control(alpha0 = 0, gamma0 = 1, spU = 0.5,
                        # inTol = 1e-8, inMaxIter = 2000,
...
control <- taro_control(alpha0 = 0, gamma0 = 0, spU = 0.5,
                        # inTol = 1e-8, inMaxIter = 2000,
                        spV=0.5, lamMaxFac = 1e2, se1 = 1)
# Weight No
...
save(list = ls(), file = sprintf('output/simulation1_%s.RData',index))

# Code generating script to run simulation on server
# sink("simulation1.sh")
# for(i in 1:50){
#   cat(sprintf('bsub -W 3:00 -q short -n 1 -M 16 -R \'rusage[mem=6]\' -o out/out_%s -e err/err_%s \"Rscript simulation1.R %s 1>Rout/out_%s 2>Rerr/err_%s\"\n',i,i,i,i,i))
# }
# sink()
...
library(kableExtra)
kbl(out_table,format = 'latex',booktabs = T,  caption = "Setting 1", linesep = "") %>%
...
kbl(out_table,booktabs = T,  caption = "Setting 1", linesep = "") %>%
  pack_rows("Full data", 1,4) %>%
  pack_rows("Missing data", 5,8) %>%
  kable_styling(latex_options = "striped", stripe_index = c(5:8)) %>%
...
plot_view <- ggplot(pldf1 %>% filter(Missing == 'No'),
                    aes(x=Method,y=value, color = Method ))+ # ,fill = Missing
...
setEPS()
postscript('plots/setting1.eps', width = 10, height = 5)
plot_view
dev.off()
